...
class Towers:
    ...
    def send_data(self, pattern: list) -> None:
        ...
        for i in range(16):
            GPIO.output(self._DATA_1, pattern[15 - i])
            GPIO.output(self._DATA_2, pattern[15 - (i + 16)])
            GPIO.output(self._DATA_3, pattern[15 - (i + 32)])
            GPIO.output(self._DATA_4, pattern[15 - (i + 48)])
...
class MasterLatch:
    ...
    Args:
        mode (str): The GPIO mode to use. Default is "BCM".
        latch (int): The GPIO pin number for the latch. Default is 21.
    """
